...
const (
	...
	ARDFEN_0        MCR0 = 0x00 << 6  //  IP RX FIFO should be read by IP Bus. AHB Bus read access to IP RX FIFO memory space will get bus error response.
	ARDFEN_1        MCR0 = 0x01 << 6  //  IP RX FIFO should be read by AHB Bus. IP Bus read access to IP RX FIFO memory space will always return data zero but no bus error response.
	ATDFEN          MCR0 = 0x01 << 7  //+ Enable AHB bus Write Access to IP TX FIFO.
	ATDFEN_0        MCR0 = 0x00 << 7  //  IP TX FIFO should be written by IP Bus. AHB Bus write access to IP TX FIFO memory space will get bus error response.
	ATDFEN_1        MCR0 = 0x01 << 7  //  IP TX FIFO should be written by AHB Bus. IP Bus write access to IP TX FIFO memory space will be ignored but no bus error response.
	HSEN            MCR0 = 0x01 << 11 //+ Half Speed Serial Flash access Enable.
	HSEN_0          MCR0 = 0x00 << 11 //  Disable divide by 2 of serial flash clock for half speed commands.
	HSEN_1          MCR0 = 0x01 << 11 //  Enable divide by 2 of serial flash clock for half speed commands.
	DOZEEN          MCR0 = 0x01 << 12 //+ Doze mode enable bit
	DOZEEN_0        MCR0 = 0x00 << 12 //  Doze mode support disabled. AHB clock and serial clock will not be gated off when there is doze mode request from system.
	DOZEEN_1        MCR0 = 0x01 << 12 //  Doze mode support enabled. AHB clock and serial clock will be gated off when there is doze mode request from system.
	...
	COMBINATIONEN_0 MCR0 = 0x00 << 13 //  Disable.
	COMBINATIONEN_1 MCR0 = 0x01 << 13 //  Enable.
	...
	SCKFREERUNEN_0  MCR0 = 0x00 << 14 //  Disable.
	SCKFREERUNEN_1  MCR0 = 0x01 << 14 //  Enable.
...
const (
	...
	CLRAHBBUFOPT_0 MCR2 = 0x00 << 11 //  AHB RX/TX Buffer will not be cleaned automatically when FlexSPI return Stop mode ACK.
	CLRAHBBUFOPT_1 MCR2 = 0x01 << 11 //  AHB RX/TX Buffer will be cleaned automatically when FlexSPI return Stop mode ACK.
	...
	SAMEDEVICEEN_0 MCR2 = 0x00 << 15 //  In Individual mode, FLSHA1CRx/FLSHA2CRx/FLSHB1CRx/FLSHB2CRx register setting will be applied to Flash A1/A2/B1/B2 seperately. In Parallel mode, FLSHA1CRx register setting will be applied to Flash A1 and B1, FLSHA2CRx register setting will be applied to Flash A2 and B2. FLSHB1CRx/FLSHB2CRx register settings will be ignored.
	SAMEDEVICEEN_1 MCR2 = 0x01 << 15 //  FLSHA1CR0/FLSHA1CR1/FLSHA1CR2 register settings will be applied to Flash A1/A2/B1/B2. FLSHA2CRx/FLSHB1CRx/FLSHB2CRx will be ignored.
	...
	SCKBDIFFOPT_0  MCR2 = 0x00 << 19 //  SCKB pad is used as port B SCK clock output. Port B flash access is available.
	SCKBDIFFOPT_1  MCR2 = 0x01 << 19 //  SCKB pad is used as port A SCK inverted clock output (Differential clock to SCKA). Port B flash access is not available.
...
const (
	...
	APAREN_0       AHBCR = 0x00 << 0 //  Flash will be accessed in Individual mode.
	APAREN_1       AHBCR = 0x01 << 0 //  Flash will be accessed in Parallel mode.
	CACHABLEEN     AHBCR = 0x01 << 3 //+ Enable AHB bus cachable read access support.
	CACHABLEEN_0   AHBCR = 0x00 << 3 //  Disabled. When there is AHB bus cachable read access, FlexSPI will not check whether it hit AHB TX Buffer.
	CACHABLEEN_1   AHBCR = 0x01 << 3 //  Enabled. When there is AHB bus cachable read access, FlexSPI will check whether it hit AHB TX Buffer first.
	...
	BUFFERABLEEN_0 AHBCR = 0x00 << 4 //  Disabled. For all AHB write access (no matter bufferable or non-bufferable ), FlexSPI will return AHB Bus ready after all data is transmitted to External device and AHB command finished.
	BUFFERABLEEN_1 AHBCR = 0x01 << 4 //  Enabled. For AHB bufferable write access, FlexSPI will return AHB Bus ready when the AHB command is granted by arbitrator and will not wait for AHB command finished.
	...
	READADDROPT_0  AHBCR = 0x00 << 6 //  There is AHB read burst start address alignment limitation when flash is accessed in parallel mode or flash is wordaddressable.
	READADDROPT_1  AHBCR = 0x01 << 6 //  There is no AHB read burst start address alignment limitation. FlexSPI will fetch more datas than AHB burst required to meet the alignment requirement.
)
...
const (
	...
	CSINTERVALUNIT_0 FLSHCR1 = 0x00 << 15   //  The CS interval unit is 1 serial clock cycle
	CSINTERVALUNIT_1 FLSHCR1 = 0x01 << 15   //  The CS interval unit is 256 serial clock cycle
...
const (
	...
	WMOPT1_0 FLSHCR4 = 0x00 << 0 //  DQS pin will be used as Write Mask when writing to external device. There is no limitation on AHB write burst start address alignment when flash is accessed in individual mode.
	WMOPT1_1 FLSHCR4 = 0x01 << 0 //  DQS pin will not be used as Write Mask when writing to external device. There is limitation on AHB write burst start address alignment when flash is accessed in individual mode.
	...
	WMENA_0  FLSHCR4 = 0x00 << 2 //  Write mask is disabled, DQS(RWDS) pin will be un-driven when writing to external device.
	WMENA_1  FLSHCR4 = 0x01 << 2 //  Write mask is enabled, DQS(RWDS) pin will be driven by FlexSPI as write mask output when writing to external device.
	...
	WMENB_0  FLSHCR4 = 0x00 << 3 //  Write mask is disabled, DQS(RWDS) pin will be un-driven when writing to external device.
	WMENB_1  FLSHCR4 = 0x01 << 3 //  Write mask is enabled, DQS(RWDS) pin will be driven by FlexSPI as write mask output when writing to external device.
)
...
const (
	...
	IPAREN_0 IPCR1 = 0x00 << 31  //  Flash will be accessed in Individual mode.
	IPAREN_1 IPCR1 = 0x01 << 31  //  Flash will be accessed in Parallel mode.
)
...
const (
	...
	RXDMAEN_0 IPRXFCR = 0x00 << 1 //  IP RX FIFO would be read by processor.
	RXDMAEN_1 IPRXFCR = 0x01 << 1 //  IP RX FIFO would be read by DMA.
...
const (
	...
	TXDMAEN_0 IPTXFCR = 0x00 << 1 //  IP TX FIFO would be filled by processor.
	TXDMAEN_1 IPTXFCR = 0x01 << 1 //  IP TX FIFO would be filled by DMA.
